...
- source_spec: `_bmad-output/specs/spec-repo-structure/stories/2-portal-scaffold.md`
  summary: Update `AGENTS.md` for the portal — it still says only `apps/www` has code, and its "Running and verifying" / "Where things are" sections carry no portal recipe, README pointer, or portal-specific rules (DDEV owns `.env`; `ddev restart` before `portal-setup` when `.env` is missing).
  evidence: Review confirmed the stale claims in `AGENTS.md` after the scaffold landed; agent-context documents are deferred from builds by policy, and `bmad-project-context` owns that file.
...
- source_spec: `_bmad-output/specs/spec-repo-structure/stories/2-portal-scaffold.md`
  summary: Add `ddev_version_constraint: ">= 1.25"` to both `apps/www/.ddev/config.yaml` and `apps/portal/.ddev/config.yaml`, matching the DDEV >= 1.25 prerequisite both READMEs state.
  evidence: Review noted nothing stops an older DDEV from starting either project and failing confusingly; the same gap exists in `apps/www`, so the consistent fix spans both apps and exceeds story 2's scope.
